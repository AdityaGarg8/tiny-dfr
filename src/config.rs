...
fn load_config(width: u16) -> (Config, Vec<FunctionLayer>) {
    ...
    let mut app_layer1_keys = base.app_layer_keys1.unwrap();
    let mut app_layer2_keys = base.app_layer_keys2.unwrap();
    let mut app_layer3_keys = base.app_layer_keys3.unwrap();
    if width >= 2170 {
        let mut layers_vec = vec![
    ...
    let app_layer1 = FunctionLayer::with_config(app_layer1_keys);
    let app_layer2 = FunctionLayer::with_config(app_layer2_keys);
    let app_layer3 = FunctionLayer::with_config(app_layer3_keys);
